...
class Config:
    ...
    seeds: Tuple[int, ...] = (0, 1, 2)                   # increase for stronger robustness stats
...
def resolve_data_root() -> str:
    ...
    if not kaggle_json.exists() and not has_env:
        raise RuntimeError(
            "Kaggle credentials not found. Either set BRAIN_TUMOR_DATA_ROOT to a local dataset path, "
            "or configure Kaggle credentials (create ~/.kaggle/kaggle.json or set KAGGLE_USERNAME and KAGGLE_KEY)."
        )
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to import kagglehub. Install it (pip install kagglehub) or set BRAIN_TUMOR_DATA_ROOT."
        ) from e
    ...
    for cls_idx, cls_name in enumerate(class_names):
        ...
        for kind, picked in [("correct", correct_idxs), ("incorrect", incorrect_idxs)]:
            for rank, global_i in enumerate(picked, start=1):
                ...
                ensure_dir(save_dir)
                out_path = save_dir / f"{cls_name}_{kind}_{rank}.png"
                plt.tight_layout()
